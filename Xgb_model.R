...
test<-read.csv("test_ind.csv")

#Create new column in test 
test$status_group <- 0

#Read in train and label datasets
train<-read.csv("train_ind.csv")
label<-read.csv("train_lab.csv")
...
for (i in 2:12){
  ...
  model <- xgboost(data = train.DMatrix, objective = "multi:softmax", booster = "gbtree",
                   eval_metric = "merror", nrounds = xgb.tab$niter, 
...
write.csv(solution, file = "Water_xgb_11.csv", row.names = FALSE)
